  ...
  public NotNull(List<Operand> parameters) {
    ...
    if (this.parameters.size() > 3)
      throw new IllegalArgumentException(
          StringFormater.format("Function '{}' need 1 to 3 parameters!", name));
    if (parameters.size() > 1) {
      defaultValue = parameters.get(1).operate((EntityMap) null);
    }
    if (parameters.size() > 2) {
      Object object = parameters.get(2).operate((EntityMap)null);
