...
public class RowTransposition extends DecorateFunction {
	...
	public RowTransposition(List<Operand> parameters) {
		...
		if (parameters.size() == 0) {
			throw new IllegalArgumentException("Invalid function format! The format should be 'rowTranspose(headerColumn[,distinctHeader,valueColumns,valueColumnsName,groupColumns])'");
		}
	...
	protected RecordSet transpose(RecordSet recordSet) {
		...
		return packRecordSet(
				recordSet.getStart(), recordSet.getEnd(), transRecords);
	}
	...
	protected void extractHeaderColumnIndex(RecordSetDefinition recordSetDefinition) {
		...
		if (headerColumnIndex == -1) {
			throw new IllegalArgumentException(StringFormater.format("RecordSet has no column named '{}'", headerColumn));
		}
	}
	...
	protected void extractGroupColumnIndexes(RecordSetDefinition recordSetDefinition) {
		...
		for(int i = 0; i < groupColumns.length; i++) {
			...
			if (index == -1)
				throw new IllegalArgumentException(StringFormater.format("RecordSet has no column named '{}'", groupColumns[i]));
			groupColumnIndexes[i] = index;
		}
	}
	
	protected void extractValueColumns(RecordSetDefinition recordSetDefinition) {
		List<String> columns = new ArrayList<String>(recordSetDefinition.getColumns().size());
	...
	protected void extractValueColumnIndexes(RecordSetDefinition recordSetDefinition) {
		...
		for(int i = 0; i < valueColumns.length; i++) {
			...
			if (index == -1)
				throw new IllegalArgumentException(StringFormater.format("RecordSet has no column named '{}'", valueColumns[i]));
	...
	protected RecordSet packRecordSet(Date start, Date stop, List<TransposedRecord> transRecords) {
		RecordSetMetadata recordSetMetadata = buildTransposedRecordSetMetadata(name);
	...
	protected RecordSetMetadata buildTransposedRecordSetMetadata(String name) {
		...
		if (groupColumns != null) {
			for(int i = 0; i < groupColumns.length; i++) {
				ColumnDefinition column = new ColumnMetadata(groupColumns[i], groupColumns[i]);
				groups.add(column);
			}
		}
		columns.addAll(groups);
		if (valueColumns.length > 1) {
			ColumnDefinition column = new ColumnMetadata(valueColumnsName, valueColumnsName);
			columns.add(column);
		}
		for(int i = 0; i < transposedColumns.length; i++) {
			ColumnDefinition column = new ColumnMetadata(transposedColumns[i], transposedColumns[i]);
			columns.add(column);
		}
	...
	protected class TransposedRecord {
		...
		public void transpose(Object[] record) {
			...
			for(int i = 0; i < valueColumnIndexes.length; i++) {
				records[i][transposedColumnIndex+offset] = record[valueColumnIndexes[i]];
			}
		}
		...
		public List<Object[]> toRecords() {
			...
			for(int i = 0; i < records.length; i++) {
				...
				if (groupKey != null) {
					System.arraycopy(groupKey.getGroups(), 0, record, 0, groupKey.getGroups().length);
