...
public class TestMoqlTranslator extends TestCase {
	...
	protected void testSqlDialect(String sql) {
		try {
			SelectorDefinition selectorDefiniton = MoqlUtils.parseMoql(sql);
			String moql = MoqlUtils.translateMetadata2Sql(selectorDefiniton, sqlDialectType);
			moql = moql.trim();
	...
	public void testTwoSingleTableSelector() {
		String sql = "select cache(100,lfu) a.id, a.name, a.num % 500, b.id, b.name, b.num % 500 " +
				"from BeanA a, BeanB b where a.num % 500 > 10 or b.num % 500 < 400 order by 3 desc, 6 desc";
		testSqlDialect(sql);
	}
	
	public void testTwoLeftJoinTableSelector1() {
		String sql = "select cache(100,filo) a.id, a.name, a.num % 500, b.id, b.name, b.num " +
	...
	public void testTwoLeftJoinTableSelector2() {
		String sql = "select cache(100,fifo) a.id, a.name, a.num % 500, b.id, b.name, b.num " +
				"from BeanA a left join BeanB b on a.id = b.id where a.num % 500 > 10 or b.num < 400 order by 3 desc";
		testSqlDialect(sql);
	}
	...
	public void testThreeSingleTableSelector() {
		String sql = "select cache(100,fifo) a.id, a.name, a.num, b.id, b.name, b.num, c.id, c.name, c.num " +
	...
	public void testConditionSelector() {
		String sql = "select cache(100,fifo) a.id, a.name, a.num, b.id, b.name, b.num " +
	...
	public void testValueSelector() {
		String sql = "select cache(100,fifo) (select cache(100,fifo) b.id from BeanB b) id, a.name " +
				"from BeanA a";
		testSqlDialect(sql);
	}
	...
	public void testTwoUnionSelector() {
		String sql = "select cache(100,fifo) a.id id, a.name name, a.num num from BeanA a " +
