...
public abstract class ResourceUtils {
	...
	public static URL getURL(String resourceLocation)
		...
		if (resourceLocation.startsWith(CLASSPATH_URL_PREFIX)) {
			String path = resourceLocation.substring(CLASSPATH_URL_PREFIX
					.length());
			URL url = ClassLoaderUtils.getDefaultClassLoader()
					.getResource(path);
			if (url == null) {
				throw new FileNotFoundException(
						StringFormater.format("Resource location '{}'" +
								"cannot be resolved to URL because it does not exist!", resourceLocation)
								);
			}
			return url;
		}
		...
		try {
			...
			try {
				...
				throw new FileNotFoundException(
						StringFormater.format("Resource location '{}'" +
								" is neither a URL not a well-formed file path", resourceLocation));
			}
		}
	}
	...
	public static File getFile(String resourceLocation)
		...
		if (resourceLocation.startsWith(CLASSPATH_URL_PREFIX)) {
			...
			if (url == null) {
				throw new FileNotFoundException(
						StringFormater.format("'{}' cannot be resolved to absolute file path " +
						"because it does not reside in the file system", resourceLocation));
			}
			return getFile(url);
		}
	...
	public static File getFile(URL resourceUrl) throws FileNotFoundException {
		if (!URL_PROTOCOL_FILE.equals(resourceUrl.getProtocol())) {
			throw new FileNotFoundException(StringFormater.format("'{}' cannot be resolved to absolute file path " +
					"because it does not reside in the file system", resourceUrl.toString()));
		}
		try {
	...
	public static File getFile(URI resourceUri) throws FileNotFoundException {
		if (!URL_PROTOCOL_FILE.equals(resourceUri.getScheme())) {
			throw new FileNotFoundException(StringFormater.format("'{}' cannot be resolved to absolute file path " +
					"because it does not reside in the file system", resourceUri.toString()));
		}
